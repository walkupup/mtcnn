...
void MTCNN::generateBbox(ncnn::Mat score, ncnn::Mat location, std::vector<Bbox>& boundingBox_, float scale){
    ...
    for(int row=0;row<score.h;row++){
        for(int col=0;col<score.w;col++){
            if(*p>threshold[0]){
                bbox.score = *p;
                bbox.x1 = round((stride*col)*inv_scale);
                bbox.y1 = round((stride*row)*inv_scale);
                bbox.x2 = round((stride*col+cellsize - 1)*inv_scale);
                bbox.y2 = round((stride*row+cellsize - 1)*inv_scale);
...
void MTCNN::testNet() {
	...
	ex.extract("prob1", score_);
	//ex.extract("conv4-2", location_);
	cout << (float)score_[0] << ", " << (float)score_[1] << endl;
	cout << score_.channel(0)[0] << ", " << score_.channel(1)[0] << endl;
	return;
}
...
void MTCNN::PNet(){
	...
	float m_min = (float)MIN_DET_SIZE / maxsize;
    minl *= m;
    float factor = pre_facetor;
    vector<float> scales_;
    while(minl>MIN_DET_SIZE && m > m_min)
	{
        scales_.push_back(m);
        minl *= factor;
        m = m*factor;
    ...
    for (size_t i = 0; i < scales_.size(); i++) {
        ...
        ncnn::Mat in;
        resize_bilinear(img, in, ws, hs);
		//show_ncnnMat(img, "img", 127.5, 1/ 0.0078125);
		//cv::waitKey();
        ncnn::Extractor ex = Pnet.create_extractor();
...
void MTCNN::ONet(){
    ...
    for(vector<Bbox>::iterator it=secondBbox_.begin(); it!=secondBbox_.end();it++){
		...
		int y1 = 117;
		int y2 = 205;
		//copy_cut_border(img, tempIm, y1, img_h - y2, x1, img_w - x2);
		copy_cut_border(img, tempIm, (*it).y1, img_h - (*it).y2, (*it).x1, img_w - (*it).x2);
        ncnn::Mat in;
        resize_bilinear(tempIm, in, 48, 48);

		//show_ncnnMat(in, "in", 127.5, 1 / 0.0078125);
		...
		cout << "score " << (float)score[1] << endl;
		if ((float)score[1] > threshold[2]) {
			for (int channel = 0; channel < 4; channel++) {
				it->regreCoord[channel] = (float)bbox[channel];
				//cout << it->regreCoord[channel] << endl;
			}
			it->area = (it->x2 - it->x1) * (it->y2 - it->y1);
			//it->score = score[1];// score.channel(1)[0];
			it->score = 0;// score.channel(1)[0];
...
void MTCNN::detect(ncnn::Mat& img_, std::vector<Bbox>& finalBbox_, std::vector<Bbox>& rawBbox_){
    ...
    refine(secondBbox_, img_h, img_w, false);
